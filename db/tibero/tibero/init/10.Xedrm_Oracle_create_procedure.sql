...
create or replace TRIGGER XEDRM5.TR_DELETE_CACHE_ACL
...
create or replace TRIGGER XEDRM5.TR_DELETE_SECUREACCESS
...
create or replace TRIGGER XEDRM5.TR_INSERT_CACHE_ACL
...
--------------------------------------------------------
 ...
 create or replace TRIGGER XEDRM5.TR_INSERT_SECUREACCESS 
...
BEGIN 
	...
	-- 1. 자식 권한 삭제
	DELETE FROM XEDRM5.ES_EFFECTIVE_ACCESS 
	WHERE ELEMENTID IN (
		-- 자식값 추출
    ...
    loop
        ...
        INSERT INTO XEDRM5.es_effective_access (ELEMENTID, USERROLE, PRIVILEGE, TEMPLATEID, PRIVILEGETYPE, DESCR, STARTDATE, ENDDATE, INDEXED, PRIV_ID) 
		...
		FROM XEDRM5.ES_EFFECTIVE_ACCESS where pid=elementid;
...
CREATE OR replace FUNCTION XEDRM5.FUNC_GET_GROUP_PERMISSION_PRIV (f_bit number , f_userId CHAR, f_elementId CHAR)
	RETURN NUMBER
IS
 priv_num NUMBER := 0;
...
CREATE OR REPLACE FUNCTION XEDRM5.func_get_permission_priv(f_bit numeric, f_userid character, f_elementid character)
 RETURN numeric
is
	...
	if need_chk_subgroup = 0 then
        ...
        from (
             select * from sys_group_member where group_id in(
                 with childlist(group_id, parent_id, path, cycle) as (
                     select group_id, parent_id, '"'||group_id||'"' as path, 'F'
                     from sys_group sg
                     where group_id = group_str
                     union all
                     select c.group_id,c.parent_id, p.path||'/"'||c.group_id||'"' as path,  case when INSTR(p.path, '"'||c.group_id||'"') = 0 then 'F' else 'T' end as cycle
                     from sys_group c, childlist p
                     where c.parent_id = p.group_id and  cycle='F'
                 )
                 select group_id
                 from childlist
             ) and member_id = f_userId
