...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PROCEDURE ES_ADMINDELGROUP 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PROCEDURE ES_ADMINDELUSER 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PROCEDURE MOVE_FOLDER 
...
--0. 변수값 저장
    ...
    SELECT parentACLid 
        INTO s_parentACLID
    FROM asyselement where elementid = sElementId;

    -- t_userSClass
    SELECT usersclass 
        INTO t_userSClass
    FROM asyselement where elementid = tElementId;
...
-- parentId가 null인 엘리먼트는 갱신 안함.
    update asyselement set parentid = tElementId
...
--3. parentACLid 변경
 /* soure의 parentACLid 값이 존재할 때,
    ...
    if (s_parentACLID is not null) then
        update asyselement E set parentACLid = t_parentACLID 
        where parentACLid = s_parentACLID
        and usersclass = 'INHERITED       '
        and exists (select elementid from asyselement where path like s_path);
...
--4. folderPath 변경
  ...
  update asysElement set folderpath = replace(folderpath, s_folderpath, t_folderpath)
  where path like s_path;    

--6. groupId 변경  
  update asysElement set groupId = tGroupId
  where path like s_path;
    
--5. path 변경
  s_path_length := length(s_path) - 17;

  update ASYSELEMENT aa
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PROCEDURE SP_ASYSADDCONTENTELEMENT 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PROCEDURE SP_ASYSADDCONTENTELEMENTEX 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PROCEDURE SP_ASYSADDELEMENT 
...
--------------------------------------------------------
  ...
  CREATE OR REPLACE PROCEDURE SP_ES_DELELEMENT 
